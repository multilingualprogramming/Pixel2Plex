...
function countPolygonsByType(svgContent) {
  ...
  const counts = { triangles: 0, hexagons: 0, other: 0 };
...
for (let i = 0; i < files.length; i++) {
  ...
  try {
    ...
    const total = counts.triangles + counts.hexagons + counts.other;

    console.log(`${file.name}`);
    console.log(`  Expected: ${file.expected}`);
    console.log(`  Generated: ${counts.triangles} tri + ${counts.hexagons} hex + ${counts.other} other = ${total}`);
